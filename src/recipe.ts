...
export class Recipe implements RecipeInterface {
  ...
  renderTooltip(extra) {
    ...
    if (
      ...
      this.products[0].item.name === this.name &&
...
function makeRecipe(data, items, d) {
  ...
  for (let { name, amount } of d.ingredients) {
    ...
    ingredients.push(
      new Ingredient(item, Rational.from_float_approximate(amount))
    )
  }
  let conditions = []
...
function getSteam(data) {
  ...
  let tempDelta = R(boilerDef.target_temperature).sub(
    R(water.default_temperature)
  )
