...
export let MODIFICATIONS = new Map([
  ['2-0-10', new Modification('Alpha 2 Phase 1', 'vanilla-2.0.10.json', false)],
])
...
export function renderDataSetOptions(settings) {
  ...
  for (let [modName, mod] of MODIFICATIONS) {
    ...
    if (
      (configuredMod && configuredMod === modName) ||
      (!configuredMod && modName === DEFAULT_MODIFICATION)
    ) {
    ...
    planetDiv
      ...
      .on('click', function (event, d) {
        if (event.shiftKey) {
          ...
          d3.selectAll('#planet_selector .toggle').classed('selected', (d) =>
            spec.selectedPlanets.has(d)
          )
        }
        d3.selectAll('#recipe_toggles .toggle').classed(
          'selected',
          (d) => !spec.disable.has(d)
        )
        spec.updateSolution()
      })
      .append((d) => d.icon.make(32))
