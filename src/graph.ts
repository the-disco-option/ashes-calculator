...
export function getColorMaps(nodes, links) {
  ...
  for (let node of nodes) {
    ...
    if (
      ...
      itemColors.has(recipe.products[0].item)
    ) {
...
export function renderNode(
  ...
  rects
    ...
    .attr('fill', (d) =>
      d3
        .color(colorList[recipeColors.get(d.recipe) % colorList.length])
        .darker()
    )
    .attr(
      'stroke',
      (d) => colorList[recipeColors.get(d.recipe) % colorList.length]
    )
    .each(function (d) {
      d.element = this
    })
  ...
  labeledNode
    ...
    .attr(
      ...
      (d) =>
        ...
        nodeMargin +
        iconSize +
        (d.building === null ? 0 : colonWidth + iconSize) /*+ 5*/
    )
    .attr('y', (d) => (d.y0 + d.y1) / 2)
    .attr('dy', '0.35em')
    .text((d) => d.text())
